...
#load monet
monet = []
for path in [f for f in os.listdir(monet_dir) if f[-4:] == ".jpg"]:
    #read jpg
    monet.append(cv2.imread ( monet_dir + path ))
    print("loading image " + path[:-4])

#load photo
photo = []
for path in [f for f in os.listdir(photo_dir) if f[-4:] == ".jpg"]:
    #read jpg
    photo.append(cv2.imread ( photo_dir + path ))
    print("loading image " + path[:-4])

#%%
#read in images with tensorflow
GCS_PATH = os.getcwd() + '/data'
...
# example_monet = monet[100]
# example_photo = photo[100]
...
# monet_ds = []
# for img in monet:
#     monet_ds.append((np.float32(img)/127.5)-1)
#
# photo_ds = []
# for img in photo:
#     photo_ds.append((np.float32(img)/127.5)-1)
#
# monet_ds = np.array(photo_ds)
...
def downsample(filters, size, apply_instancenorm=True):
    ...
    #use a convolution to downsample in D instead of pooling layer
    result.add(layers.Conv2D(filters, size, strides=2, padding='same',
                             kernel_initializer=initializer, use_bias=False))

    #use batch normalization or instance normalization
...
def upsample(filters, size, apply_dropout=False):
    ...
    result = keras.Sequential()
    #use a transposed convolutional layer in G instead of pooling layer
